...
function withDemoUser(headers: HeadersInit = {}, demoUserId?: string): HeadersInit {
  return demoUserId
    ? {
        ...headers,
...
export async function getRequestById(id: string, demoUserId: string) {
  return request<RequestResponseDto>(
    `${REQUESTS_PATH}/${encodeURIComponent(id)}`,
    {
      headers: withDemoUser({}, demoUserId),
    }
  );
}
...
export async function removeRequest(id: string, demoUserId: string) {

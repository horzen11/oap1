...
export const requestsService = {
  ...
  async searchUnsafe(query: Record<string, unknown>) {
    ...
    return requestsRepository.searchUnsafe(q);
  },
  ...
  async update(id: string, dto: CreateRequestRequestDto): Promise<EquipmentRequest> {
    ...
    return (await requestsRepository.update(id, updated)) ?? updated;
  },

  async patch(id: string, dto: PatchRequestRequestDto): Promise<EquipmentRequest> {
    const existing = await this.getById(id);
  ...
  async delete(id: string): Promise<void> {
    const deleted = await requestsRepository.delete(id);

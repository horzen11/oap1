...
export const requestsService = {
  async getAll(query: Record<string, unknown>, ownerUserId: string) {
    ...
    let items = await requestsRepository.getAllOwned(ownerUserId, search, status);
  ...
  async getById(id: string, ownerUserId?: string): Promise<EquipmentRequest> {
    const request = ownerUserId
      ? await requestsRepository.getByIdForOwner(id, ownerUserId)
      : await requestsRepository.getById(id);
    if (!request) throw new ApiError(404, "NOT_FOUND", "Request not found");
    return request;
  },

  async searchSafe(query: Record<string, unknown>, ownerUserId: string) {
    const q = typeof query.q === "string" ? query.q : "";
    return requestsRepository.searchSafe(q, ownerUserId);
  },
  ...
  async getFullStats(query: Record<string, unknown>) {
    ...
    const totalComments = rows.reduce((sum, row) => sum + Number(row.commentsCount), 0);
  ...
  async create(dto: CreateRequestRequestDto, ownerUserId: string): Promise<EquipmentRequest> {
    validateRequestDto(dto);
    const user = await usersService.getById(dto.userId);
    await usersService.getById(ownerUserId);
  ...
  async update(id: string, dto: CreateRequestRequestDto, ownerUserId: string): Promise<EquipmentRequest> {
    const existing = await this.getById(id, ownerUserId);
    validateRequestDto(dto);
    const user = await usersService.getById(dto.userId);
    ...
    const result = await requestsRepository.update(id, ownerUserId, updated);
    if (!result) throw new ApiError(404, "NOT_FOUND", "Request not found");
    return result;
  },

  async patch(id: string, dto: PatchRequestRequestDto, ownerUserId: string): Promise<EquipmentRequest> {
    const existing = await this.getById(id, ownerUserId);
    ...
    return this.update(id, merged, ownerUserId);
  },

  async delete(id: string, ownerUserId: string): Promise<void> {
    const deleted = await requestsRepository.delete(id, ownerUserId);
